# FishingTweaks  
Changes the fishing catch timer to 60 seconds so you have time to alt+tab back in. Also lets you fish from people's heads and desks etc. Now with optional auto-fishing (split into auto-catch, auto-recast, and infinite bait).   

# Commands  
- /help fish to view all possible commands.  
- /autocatch - enable automatic minigame completion. default: off.  
- /autorecast - enable recasting to your last position. cancel by stowing the fishing rod. default: off.  
- /infinitebait - enable/disable the infinite bait. default: off.
- /mutefishing - gets rid of the splash sfx when bobbers hit water. default: off.  
- /forcenormalsizes - rerolls fish size visuals to keep oversized fish off your screen. default: off.
  
Manual Installation:  
1. Install BepInEx as directed in the main repository instructions.  
2. Extract archive from RELEASES to your game's installation, making sure to overwrite/merge the folders as prompted.
