...
namespace ChatTweaks.patches
{
    ...
    internal class UIManagerPatch
    {
        ...
        UIManager __instance)
        {
            ...
            ____messagePlaceholderText.color = ____messagePlaceholderActiveColor;//DOColor(____messagePlaceholderActiveColor, ____uiFadeDuration);
            ...
            for (int i = 0; i < ____messagePanelImages.Count; i++)
            {
                ...
                color.a = a;
                ____messagePanelImages[i].color = color;//DOColor(color, ____uiFadeDuration);
            }
            for (int j = 0; j < ____messagePanelTexts.Count; j++)
            {
                Color color2 = ____messagePanelTexts[j].color;
                color2.a = a;
                ____messagePanelTexts[j].color = color2; //DOColor(color2, ____uiFadeDuration);
            }
            return false;
        }
